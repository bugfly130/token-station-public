...
export default cachedEventHandler(
  async (event) => {
    ...
    const network =
      env === 'mainnet' ? Network.MainnetSentry : Network.TestnetSentry
    ...
    tokensChain.supply = [
      ...denoms
        .filter((x) => !tokensChain.supply.some((y) => y.denom === x.denom))
        .map((x) => ({ denom: x.denom, amount: '0' })),
      ...tokensChain.supply
    ]

    await demonClient.preloadMetadata()

    const allTokensAsync = tokensChain.supply
